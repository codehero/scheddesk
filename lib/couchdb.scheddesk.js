				...
				function(err, stateQuery){
					...
					for(var i = 0; i < stateQuery.length; ++i){
						...
						if(TRIGGER_BIT.DEQUEUED & stateQuery[i].value){
							++runningCount;
							var id = "scheddesk.forcedAbort."+ ts +"."+ runningCount;
...
CouchDatabase.prototype._executeTrigger = function(triggerID, dequeueID){
	...
	taskDef.get(function(err, task){
		...
		if(task.taskType == "child_process"){
			...
			var options = {
				"cwd": (trig.params && trig.params.cwd) || task.params.cwd || this._conf.defaultCWD,
			...
			if("tempFiles" in task){
				var base = path.join(this._conf.tmpDir, "TMPFILE_" + triggerID);
				for(var i = 0; i < task.tempFiles.length; ++i){
					options.env["TMPFILE_" + i] = base + "." + i;
				}
			}
			if("tempDirs" in task){
				var base = path.join(this._conf.tmpDir, "TMPDIR_" + triggerID);
						...
						errDoc.save(function(err, d){
							...
							if(err){
								...
								if(task.tempFiles || task.tempDirs){
									var baseFile =
										path.join(this._conf.tmpDir, "TMPFILE_" + triggerID);
									var baseDir =
										path.join(this._conf.tmpDir, "TMPDIR_" + triggerID);

									function saveAttachment(counter){
										if(counter == task.tempFiles.length + task.tempDirs.length)
											return;

										if(counter < task.tempFiles){
											/* Make sure that temp file exists. */
											var filename =
												path.join(this._conf.tmpDir, baseFile + "." + counter);
											try{
											...
											var mime = diag.mime;
											var attach = doc.attachment(afile);
											...
											var dirCounter = counter - task.tempFiles.length;
											...
											attach.setBody(mime, child_process.stdout);
											attach.save(function(err, response){
												if(err){
													/* TODO Sa*/
													util.debug("Saving err attachment: " + JSON.stringify(err));
			...
			if("email" in trig){
				...
				smtpConfig.get(function(err, cfg){
					...
					nodemailer.send_email(email, function(error, success){
							...
							if(error){
								var errID = "scheddesk.notificationError." + ts;
								var errDoc = cdb.doc(errID);
								errDoc.body = {
									"_id" : errID,
									"email" : email,
