...
var CouchDatabase = function(conf, seq){
	...
	this._conf = conf;
	this._couchClient = couchdb.createClient(conf.port, conf.host, conf.user, conf.password);
									...
									if("scheddesk_retry" in doc){
										_this._db.getDoc(doc.scheddesk_retry.triggerID, function(err, data){
											if(err){
												...
												_this._triggers[data._id] = data.scheddesk_trigger;
				...
				if("scheddesk_err_report" in doc){
					if(_this._executingTrigger){
						if(doc.scheddesk_err_report.triggerID in _this._triggers){
							_this.emit("terminatedRequest", data);
							/* Remove from _triggers. */
							delete _this._triggers[doc.scheddesk_err_report.triggerID];
							_this._executingTrigger = null;
						}
						else{
							/* BUG */
...
CouchDatabase.prototype._beginInit = function(){
	...
	cdb.view("SchedDeskApp", "Schedule",
		...
		function(err, scheduleRes){
			if(err){
				/* TODO */
				sys.debug("ERROR1 " + JSON.stringify(err));
				return;
			}
			...
			cdb.list("SchedDeskApp", "StateQuery", "State",
				...
				function(err, stateQuery){
					if(err){
						/* TODO */
						sys.debug("ERROR2 " + JSON.stringify(err));
						return;
					}
					...
					sys.debug("STATE QUERY: " + JSON.stringify(stateQuery));
					...
					if(ids.length){
						_this._db.getMultipleDocs(ids, function(err, mdocs){
							if(err){
							sys.debug("ERROR3 " + JSON.stringify(err));
								/* TODO */
								return;
							}
...
CouchDatabase.prototype._executeTrigger = function(triggerID, dequeueID){
	...
	if(trig.taskDefinition == "scheddesk.retry"){
		...
		cdb.saveDoc(retryDoc, function(err, data){
			/* Updated State, have to recalculate triggers. */
			if(err){
				sys.debug("Save retry " + JSON.stringify(err));
			}
		});
	...
	cdb.getDoc(trig.taskDefinition, function(err, task){
		...
		if(task.taskType == "child_process"){
			...
			sys.debug("EXECUTING " + task.params.command);
			proc.on("exit", function(code){
				sys.debug("Exited with code " + code);
				if(!task.taskType.filesTermination){
					...
					if(!code){
						...
						cdb.saveDoc(errDoc, function(err, d){
							sys.debug("Saved error report: ");
							if(err){
								/* TODO */
								sys.debug("Saving termination error: " + JSON.stringify(err));
							}
								...
								if(task.diagnosticFiles){
									...
									sys.debug(JSON.stringify(_this));
									sys.debug(JSON.stringify(_this._conf));
									sys.debug(prefix);
									...
									function saveAttachment(counter, rev){
										...
										child_process.exec(prefix + suffix, function(err, stdout, stderr){
											if(err){
												/* TODO Sa*/
												sys.debug("Saving err attachment: " + JSON.stringify(err));
											}
											else{
...
CouchDatabase.prototype._determineNextTrigger = function(){
	...
	if(this._dequeuingCount || this._executingTrigger){
		sys.debug("_dequeuingCount:" + this._dequeuingCount);
		sys.debug("_executingTrigger:" + this._executingTrigger);
		return;
	}

	if(this._dequeueTimeout){
		sys.debug("Clearing dequeue");
		...
		if(!(routineID in checkoff)){
			...
			switch(r.period.units){
				...
				case "days":
					...
					if(trigger.scheddesk_trigger.scheduledTime < now){
						sys.debug("PUSHING FORWARD");
						trigger.scheddesk_trigger.scheduledTime += 86400000;
					}
					break;
