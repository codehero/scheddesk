						...
						cdb.saveDoc(errDoc, function(err, d){
							...
							if(err){
								...
								var prefix = "curl --user "
									+ _this.conf.user+":"+ _this.conf.password
									+"-vX PUT http://"+ this._conf.hostname +":"+ this._conf.port
								...
								function saveAttachment(counter, rev){
									...
									var filename = task.diagnosticFiles[i].path.split("/");
									filename = filename.length ?
										filename[filename.length - 1] : task.diagnosticFiles[i];
									var mime = task.diagnosticFiles[i].mime;
									var suffix = "/"+ filename +"?rev="+ rev
										+" -H \"Content-Type: \""+ mime
										+"\" --data-binary @" + task.diagnosticFiles[i];

									child.exec(prefix + suffix, function(err, stdout, stderr){
										if(err){
											/* TODO Sa*/
