...
CouchDatabase.prototype._executeTrigger = function(triggerID){
	...
	cdb.getDoc(_this._triggers[triggerID].taskDefinition, function(err, task){
		...
		if(task.taskType == "child_process"){
			...
			var proc = child_process.spawn(task.params.command, task.params.args, options);
...
CouchDatabase.prototype._determineNextTrigger = function(){
	...
	if(target){
		...
		if(_this._triggers[target].scheduledTime * 1000 < tf){
			cb();
		}
		else{
			_this.emit("scheduling", _this._triggers[target]);
			var diff = _this._triggers[target].scheduledTime * 1000 - tf;
		...
		if(!(routineID in checkoff)){
			...
			switch(r.period.units){
				case "seconds":
					...
					var t = Math.ceil(diff / r.period.magnitude);
					trigger.scheddesk_trigger.scheduledTime = t * r.period.magnitude + r.interval[0];
				...
				case "days":
					...
					if(r.period.time.isClock){
						...
						trigger.scheddesk_trigger.scheduledTime = Math.round(dn.getTime() / 1000);
					}
					else{
						var dn = new Date(d.getYear(), d.getMonth(), d.getDay());
						dn = Math.round(dn.getTime() / 1000);
						dn += 3600 * r.period.time.value[2] +
							60 * r.period.time.value[1] +
							r.period.time.value[0];
